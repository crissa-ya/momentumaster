...
document.getElementById('colObj1').addEventListener('change', () => {
  checkAndHandleSameSelection();
  drawOnce();
});
document.getElementById('colObj2').addEventListener('change', () => {
  checkAndHandleSameSelection();
  drawOnce();
});
...
function drawOnce() {
  ...
  const isCollisionMode = (simMode.value === 'collision');
  const id1 = isCollisionMode ? parseInt(document.getElementById('colObj1')?.value) : null;
  const id2 = isCollisionMode ? parseInt(document.getElementById('colObj2')?.value) : null;

  instances.forEach(inst => {
    if (isCollisionMode && inst.id !== id1 && inst.id !== id2) {
      return;
    }
...
function alignSelectedCollisionObjects(){
...
function checkAndHandleSameSelection() {
  if (simMode.value !== 'collision') return;

  const sel1 = document.getElementById('colObj1');
  const sel2 = document.getElementById('colObj2');
  if(!sel1 || !sel2) return;

  let id1 = parseInt(sel1.value);
  let id2 = parseInt(sel2.value);
...
function refreshCollisionSelects(){
  ...
  if(prev1 && sel1.querySelector(`option[value="${prev1}"]`)) sel1.value = prev1;
  if(prev2 && sel2.querySelector(`option[value="${prev2}"]`)) sel2.value = prev2;
}

function updateCollisionUI() {
  if (simMode.value === 'collision' && instances.length >= 1) {
    refreshCollisionSelects();
...
document.getElementById('collideBtn').onclick = () => {
  let id1 = parseInt(document.getElementById('colObj1').value);
  let id2 = parseInt(document.getElementById('colObj2').value);
  ...
  // If same object selected in both dropdowns, duplicate it
  if(id1 === id2 || !b){
    const t = findType(a.typeKey);
    if(t){
      const newId = nextId++;
      ...
      createInstancePanel(b);
      refreshCollisionSelects();
      document.getElementById('colObj1').value = a.id;
  ...
  // Set simMode to collision and redraw without changing selections
